"""One-shot, bounded smoke test of the two user-specified fixtures. Never imports workflows."""
import argparse
import base64
import hashlib
import json
...
def main():
    ...
    parser.add_argument('--safe-only',action='store_true',help='Authorized follow-up: only safe image, hard limit one request.')
    ...
    assert settings.analyst_provider=='deterministic'
    ...
    output=ROOT/('evaluation/reports/live_openai_safe_preprocessed' if args.safe_only else 'evaluation/reports/live_openai_smoke')
    ...
    result={'started_at':datetime.now(timezone.utc).isoformat(),'provider':'openai','model':'gpt-5.6-sol',
        'maximum_requests':1 if args.safe_only else 2,'sdk_retries':0,'validation_attempts':1,'requests_attempted':0,
    ...
    try:
        for name in (['01_safe.jpg'] if args.safe_only else ['01_safe.jpg','02_obvious_hazard.jpg']):
            path=fixture_root/name
            original=path.read_bytes()
            row={'image':name,'sha256':hashlib.sha256(original).hexdigest(),'bytes':len(original),
                'provider':'openai','model':'gpt-5.6-sol','requests_attempted':0,'status':'preflight',
            ...
            provider=create_safety_provider(settings)
            assert type(provider) is RealMultimodalSafetyAIProvider
            ...
            def check_request(request):
                ...
                parts=[p for item in body['input'] if isinstance(item.get('content'),list) for p in item['content'] if p.get('type')=='input_image']
                assert len(parts)==1 and base64.b64decode(parts[0]['image_url'].split(',',1)[1])==processed
